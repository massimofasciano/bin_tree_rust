use crate::{BinTreeOrderedSet, BinTree};
        ...
        <= 'e' => \
            ...
            <= 'm' => \
                ...
                <= 'o' => \
                        (('s') \
                    <= 'y'))))";
    let mut s = BinTreeOrderedSet::new();
    TEST_STR.chars().for_each(|c| s.insert(c));
    assert_eq!(s.to_tree_string(),str1);
    assert_eq!(s.len(), 14);
    let s2 = TEST_STR.chars().collect::<BinTreeOrderedSet<_>>();
    assert_eq!(s2.to_tree_string(),str1);
    assert_eq!(s2.len(), 14);
    assert_eq!(s,s2);
    let s3 = BinTreeOrderedSet::from(TEST_STR.chars().collect::<Vec<_>>());
    assert_eq!(s3.to_tree_string(),str1);
    assert_eq!(s3.len(), 14);
    assert_eq!(s3,s2);
    let s4 = BinTreeOrderedSet::from(TEST_STR.chars().collect::<BinTree<_>>());
